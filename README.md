...
Since the intent of this project is to demonstrate coding ability, for
purposes of this proof-of-concept we will set aside larger
architectural questions like whether an off-the-shelf mail system
would be a better solution. We are also putting aside things like
compatibility with existing mail services and interfaces (but see
_Future Directions_ below).

## Domain Analysis

This is an object-oriented analysis of the concepts used in the
implementation of the application.  The starting points for domain
analysis are the message, mailbox, and mail user, represented by the
domain models Box, Post, and User, respectively.
...
Postgresql is being used for the database.

### Application Architecture

Both the front-end and back-end are implemented in Python and Django,
as these are the open-source web application technologies I am
currently most familiar with. I have used the Django REST framework on
top of Django to simply the development of the back-end API.  It
handles multiple API input/output formats (JSON is the main one used
in this proof of concept), as well as the common CRUD patterns
associated with REST APIs, and support for hyperlinking and a
browsable API through HTML, which are nice advanced touches that many
so-called REST APIs cannot deliver.

The disadvantages of Django are:

- Python is not a particularly fast language.
- Most system and database calls in Django are synchronous, meaning
...
_Logging:_ No production-quality web service should be without
logging. Logging database accesses is particularly critical when
troubleshooting, since they are frequently the bottlenecks to
performance. Operational errors (i.e. internal errors, not errors that
can be associated with malformed requests) should also be logged at an
urgent level. 
...
allowing users to delete anybody's box or posts, for example. A more
fine-grained authorization system could be used to help avoid
mischief.
...
Django instances, even running on different systems.
...
mailboxes up into different systems and databases. (In the early days
of the Internet, separate servers were a certainty!). Posts should
ideally be colocated with the boxes they are delivered to, so that
back-end features involving table joins between DeliveredPost and Post
will work. One way to achieve this would be to copy the Post to
whatever systems it's needed on, and change the URL which identifies
the Post to point at the system on which the Post is hosted.
...
are not very resilient to errors, refusing to process the rest of the
box if we encounter a database error during delivery. For this, and
for other cases where we might want to continue wth processing if we
