...
def api_root(request, format=None):
    ...
    return Response({
            ...
            'delivered-posts': reverse('deliveredpost-list', request=request, format=format),
...
class PostList(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        ...
        if form.is_valid():
            kwargs = {}
            for field, query_key in [('sender', 'sender'), ('start', 'created__gte'), ('end', 'created__lte')]:
...
class DeliveredPostList(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        ...
        if form.is_valid():
            kwargs = {}
            for field, query_key in [('sender', 'post_sender'), ('boxname', 'box__name'),
                                     ('start', 'post_created__gte'), ('end', 'post_created__lte')]:
                if form.cleaned_data[field]:
                    kwargs[query_key] = form.cleaned_data[field]
            if kwargs:
                queryset = queryset.filter(**kwargs)
...
def deliver(request, format=None):
    ...
    for box in data['to']:
        dpost, created = DeliveredPost.objects.get_or_create(box=box, post=post,
                                                             post_sender=request.user,
                                                             post_created=post.created,
                                                             post_subject=post.subject)
        if created:
            dpost_serializer = DeliveredPostSerializer(dpost, context={'request': request})
...
def sync(request, format=None):
    ...
    for sub in subs:
        ...
        for dpost in dposts:
            DeliveredPost.objects.get_or_create(box=box, post=dpost.post,
