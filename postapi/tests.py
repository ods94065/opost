...
class AuthAPITestCase(django.test.TestCase):
    ...
    def test_get_allowed(self):
        ...
        for resource in ['boxes', 'posts', 'delivered-posts', 'subscriptions',
                         'boxes/123', 'posts/123', 'delivered-posts/123', 'subscriptions/123']:
            r = self.client.get(papi(resource))
            self.assertNotEqual(r.status_code, 403)

    def test_post_forbidden(self):
        '''POST requests are forbidden when unauthenticated.'''
        for resource in ['boxes', 'posts', 'delivered-posts', 'subscriptions', 'actions/deliver']:
            r = self.client.post(papi(resource), json.dumps({}), content_type='application/json')
            self.assertEqual(r.status_code, 403)
    
    def test_put_delete_forbidden(self):
        '''PUT and DELETE requests are forbidden when unauthenticated.'''
        for resource in ['boxes/123', 'posts/123', 'delivered-posts/123', 'subscriptions/123']:
    ...
    def test_basic_auth(self):
        ...
        r = self.client.post(papi('boxes'), {'name': 'foo'}, HTTP_AUTHORIZATION=basic_test_auth())
    ...
    def test_session_auth(self):
        ...
        r = self.client.post(papi('boxes'), json.dumps({'name': 'foo'}), content_type='application/json')
        self.assertEqual(r.status_code, 201)
        self.client.logout()
...
class APITestCase(django.test.TestCase):
    ...
    def setUp(self):
        self.client.login(username='test', password='testy123') # Constructed to match the fixture data!
...
class PostDetailTestCase(APITestCase, PostMixin):
    ...
    def test_put(self):
        ...
        r = self.client.put(papi('posts', self.pk), {'subject': 'Epilogue', 'body': "The drama's done."})
        self.assertEqual(r.status_code, 405)
...
class DeliveredPostMixin(BoxMixin, PostMixin):
    ...
    def deliver_post(self, box_url, post_url):
        r = self.client.post(papi('delivered-posts'), { 'box': box_url, 'post': post_url })
        self.assertEqual(r.status_code, 201)
        url = r.json_content['url']
        return (scrape_pk(url), url)
    ...
    def create_and_deliver_post(self, boxname, subject, body):
        ...
        return DeliveredPostMixin.Data(boxname, box_url, post_pk, post_url, dpost_pk, dpost_url)
...
class DeliveredPostListTestCase(APITestCase, DeliveredPostMixin):
    ...
    def test_fails_if_delivered_twice(self):
        ...
        r = self.client.post(papi('delivered-posts'), { 'box': box_url, 'post': post_url2 })
        self.assertEqual(r.status_code, 201)
...
class DeliveredPostDetailTestCase(APITestCase, DeliveredPostMixin):
    ...
    def test_mark_read_and_unread(self):
        ...
        # mark as read
        r = self.client.patch(papi('delivered-posts', self.data.dpost_pk), { 'is_read': True })
        self.assertEqual(r.status_code, 200)
        self.assertTrue(DeliveredPost.objects.get(id=self.data.dpost_pk).is_read)

        # now mark as unread again
        r = self.client.patch(papi('delivered-posts', self.data.dpost_pk), { 'is_read': False })
        self.assertEqual(r.status_code, 200)
        self.assertFalse(DeliveredPost.objects.get(id=self.data.dpost_pk).is_read)
...
class SubscriptionMixin(DeliveredPostMixin):
    ...
    This code relies on self.client being a JSONClient.'''
    class Data(object):
        def __init__(self, source_name, source_url, target_name, target_url, sub_pk, sub_url):
    ...
    def create_subscription(self, source_url, target_url):
        ...
        r = self.client.post(papi('subscriptions'), { 'source': source_url, 'target': target_url })
        self.assertEqual(r.status_code, 201)
        url = r.json_content['url']
        return (scrape_pk(url), url)
    ...
    def create_boxes_and_subscription(self, source_name, target_name):
        ...
        return SubscriptionMixin.Data(source_name, source_url, target_name, target_url, sub_pk, sub_url)
...
class SubscriptionDetailTestCase(APITestCase, SubscriptionMixin):
    ...
    def test_set_watermark(self):
        ...
        _, post_url = self.create_post('Emergency', 'This is a test of the emergency broadcast system.')
        dpost_pk, dpost_url = self.deliver_post(self.data.source_url, post_url)
        r = self.client.patch(papi('subscriptions', self.data.sub_pk), {'watermark': dpost_url})
        self.assertEqual(r.status_code, 200)
        self.assertEqual(Subscription.objects.get(id=self.data.sub_pk).watermark.id, dpost_pk)
...
class DeliverActionTestCase(APITestCase, BoxMixin):
    ...
    def test_one(self):
        ...
        r = self.client.post(papi('actions/deliver'), {'to': [self.box_url],
                                                       'subject': 'Test',
                                                       'body': 'Hello, world!'})
        self.assertEqual(r.status_code, 200)
    ...
    def test_multiple(self):
        ...
        r = self.client.post(papi('actions/deliver'), {'to': [self.box_url, box_url2],
                                                       'subject': 'Test',
                                                       'body': 'Hello, world!'})
        self.assertEqual(r.status_code, 200)
    ...
    def test_succeds_if_box_listed_twice(self):
        ...
        r = self.client.post(papi('actions/deliver'), {'to': [self.box_url, self.box_url],
                                                       'subject': 'Test',
                                                       'body': 'Hello, world!'})
        self.assertEqual(r.status_code, 200)
...
class SyncActionTestCase(APITestCase, SubscriptionMixin):
    ...
    def test_no_watermark(self):
        ...
        self.assertEqual(DeliveredPost.objects.filter(box__name=self.data.target_name).count(), 0)
    ...
    def test_with_watermark(self):
        ...
        posts = [self.create_post('Test', 'Hello, cool people!'),
                 self.create_post('Free Trashcan Mac offer for cool people only', 'Made you look!')]
        source_dposts = [self.deliver_post(self.data.source_url, post_data[1])
                         for post_data in posts]
        self.deliver_post(self.data.target_url, posts[0][1])
        r = self.client.patch(papi('subscriptions', self.data.sub_pk), {'watermark': source_dposts[0][1]})
        self.assertEqual(r.status_code, 200)
        sub = Subscription.objects.get(id=self.data.sub_pk)
